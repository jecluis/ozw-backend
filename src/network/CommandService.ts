...
abstract class Command {
    ...
    protected markIsRunning(): void { this._state.is_running = true; }
    protected markIsFailed(): void {
        this._state.is_failed = true;
        this.markNotAlive();
    }
    protected markIsWaiting(): void { this._state.is_waiting = true; }
    protected markIsCancelled(): void {
        this._state.is_cancelled = true;
        this.markNotAlive();
    }
    protected markIsCompleted(): void {
        this._state.is_completed = true;
        this.markNotAlive();
    }
    protected markNotAlive(): void {
        this._state.is_running = false;
        this._state.is_waiting = false;
    }
    protected isNotAlive(): boolean {
    ...
    public isDone(): boolean {
        const is_done: boolean = (this._is_cancelled || this._isDone());
...
class CommandHealNetwork extends Command {
    constructor() {
        ...
        const netsvc: NetworkService = NetworkService.getInstance();
    ...
    public _isDone(): boolean {
        ...
        const is_done: boolean = (this._num_finished === this._total_num_nodes);
        if (is_done) {
    ...
    public getPercent(): number {
        const total = this._total_num_nodes;
        const finished = this._num_finished;
        return Math.floor((finished / total) * 100);
    }
}


const logger: Logger = new Logger({name: "command-service"});
...
export class CommandService {
    ...
    private _archiveRunningCommand(): void {
        ...
        const cmd: CommandState = this._command_handler.getState();
    ...
    public healNetwork(): boolean {
        ...
        const nodes: NetworkNode[] = svc.getNodes();
        nodes.forEach( (node) => {
